    ...
    async run(ctx: Context, args: string[]): Promise<number> {
        ...
        return runWasm(ctx, "goslice.wasm", args)
            .then((gcode: string | null) => {
                ...
                const filename =
                    splittedFile[splittedFile.length - 1] + ".gcode"
                const url = URL.createObjectURL(
                    new File([gcode], filename, {
                        type: "text/x.gcode",
                    })
                )
